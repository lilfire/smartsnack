...
class TestAddCategory:
    def test_valid_add(self, app_ctx, translations_dir):
        ...
        row = get_db().execute("SELECT emoji FROM categories WHERE name='Drinks'").fetchone()
        assert row["emoji"] == "🧃"
...
class TestUpdateCategory:
    def test_update_emoji(self, app_ctx, translations_dir):
        ...
        from db import get_db
        row = get_db().execute("SELECT emoji FROM categories WHERE name='Snacks'").fetchone()
        assert row["emoji"] == "🍕"
...
class TestDeleteCategory:
    ...
    def test_delete_with_products_no_move_to(self, app_ctx, seed_product, translations_dir):
    ...
    def test_delete_with_move_to(self, app_ctx, seed_product, translations_dir):
        ...
        assert count >= 1
        from db import get_db
        row = get_db().execute("SELECT type FROM products WHERE id=?", (seed_product,)).fetchone()
