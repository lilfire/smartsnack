...
class TestAppErrorHandlers:
    ...
    def test_unhandled_error_returns_500(self, client):
        ...
        with patch("services.core_service.get_product_count", side_effect=RuntimeError("boom")):
...
class TestCoreHealthError:
    def test_health_db_error(self, client):
        ...
        with patch("services.core_service.get_product_count", side_effect=sqlite3.OperationalError("fail")):
            resp = client.get("/health")
            assert resp.status_code == 500
            data = resp.get_json()
