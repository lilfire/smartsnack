...
reference inside ``services.core_service`` (which the blueprint delegates
to since LSO-1679) so that the handler's ``try/except`` actually fires.
...
def patch_health_db(monkeypatch):
    """Swap the ``get_db`` reference inside ``services.core_service`` so the
    route's ``try`` block raises ``sqlite3.OperationalError`` exactly once.

    ``services.core_service`` does ``from db import get_db`` at import time,
    which means the service holds its OWN reference to the function. Patching
    ``db.get_db`` is therefore not enough — we must patch the symbol on the
    service module the blueprint delegates to.
    """
    ...
    monkeypatch.setattr(core_service, "get_db", _raise_db)
    return calls
...
def test_health_os_error_also_returns_500(live_url, monkeypatch):
    ...
    monkeypatch.setattr(core_service, "get_db", _raise_os)
    status, body, _ = _get(f"{live_url}/health")
