...
class TestTagDisplayInProduct:
    ...
    def test_product_shows_assigned_tags(self, page, api_create_product, live_url):
        ...
        _, tag = _api_raw(live_url, "/api/tags", method="POST",
                          body={"label": "browser-display"})
        product = api_create_product(name="TagDisplayProd")
        ...
        _expand_product_row(page, "TagDisplayProd")

        # Should see the tag somewhere in the expanded view
        row = page.locator(f".table-row:has-text('TagDisplayProd')").first
        expect(row).to_contain_text("browser-display")
...
class TestTagEditModal:
    ...
    def test_tag_field_visible_in_edit(self, page, api_create_product, live_url):
        """The tag field should be visible when editing a product."""
        api_create_product(name="TagEditVisible")
        page.reload(wait_until="domcontentloaded")
        page.wait_for_selector("#results-container", state="attached", timeout=10000)
        page.wait_for_function(
            "() => !document.querySelector('#results-container .loading')",
        ...
        _open_edit_form(page, "TagEditVisible")
        tag_field = page.locator("#tag-field-ed")
        expect(tag_field).to_be_visible(timeout=5000)

    def test_tag_modal_opens_on_click(self, page, api_create_product, live_url):
        """Clicking the tag field should open the tag modal."""
        api_create_product(name="TagModalOpen")
        page.reload(wait_until="domcontentloaded")
        page.wait_for_selector("#results-container", state="attached", timeout=10000)
        page.wait_for_function(
            "() => !document.querySelector('#results-container .loading')",
        ...
        _open_edit_form(page, "TagModalOpen")
        tag_field = page.locator("#tag-field-ed")
        tag_field.click()
        page.wait_for_timeout(300)
    ...
    def test_tag_modal_search_input(self, page, api_create_product, live_url):
        """The tag modal should have a search input."""
        # Pre-create a tag
        _, tag = _api_raw(live_url, "/api/tags", method="POST",
                          body={"label": "searchable-tag"})

        api_create_product(name="TagModalSearch")
        page.reload(wait_until="domcontentloaded")
        page.wait_for_selector("#results-container", state="attached", timeout=10000)
        page.wait_for_function(
            "() => !document.querySelector('#results-container .loading')",
        ...
        _open_edit_form(page, "TagModalSearch")
        ...
        modal_input.fill("searchable")
        page.wait_for_timeout(500)

        # Suggestions should appear
        suggestions = page.locator("#tag-modal-suggestions")
        expect(suggestions).to_contain_text("searchable-tag")

        _cleanup_tag(live_url, tag["id"])

    def test_create_new_tag_via_modal(self, page, api_create_product, live_url):
        """Typing a new tag name and confirming should create the tag."""
        api_create_product(name="TagModalCreate")
        page.reload(wait_until="domcontentloaded")
        page.wait_for_selector("#results-container", state="attached", timeout=10000)
        page.wait_for_function(
            "() => !document.querySelector('#results-container .loading')",
        ...
        _open_edit_form(page, "TagModalCreate")
        page.locator("#tag-field-ed").click()
        page.wait_for_timeout(300)

        # Type a new tag name
        modal_input = page.locator("#tag-modal-input")
        modal_input.fill("new-browser-tag")
        page.wait_for_timeout(300)
        ...
        tag_field = page.locator("#tag-field-ed")
        expect(tag_field).to_contain_text("new-browser-tag")

        # Cleanup
        _, tags = _api_raw(live_url, "/api/tags?q=new-browser-tag")
        for t in tags:
            _cleanup_tag(live_url, t["id"])

    def test_close_tag_modal_with_escape(self, page, api_create_product, live_url):
        """Pressing Escape should close the tag modal."""
        api_create_product(name="TagModalEsc")
        page.reload(wait_until="domcontentloaded")
        page.wait_for_selector("#results-container", state="attached", timeout=10000)
        page.wait_for_function(
            "() => !document.querySelector('#results-container .loading')",
        ...
        _open_edit_form(page, "TagModalEsc")
        page.locator("#tag-field-ed").click()
        page.wait_for_timeout(300)
