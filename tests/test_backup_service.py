...
class TestRestoreProduct:
    def test_restore_product(self, app_ctx, db, seed_category):
        ...
        cur = db.cursor()
        _restore_product(cur, {
            "type": "Snacks", "name": "Restored Item",
            "ean": "12345678", "brand": "Test",
        })
        db.commit()
        row = db.execute("SELECT name FROM products WHERE name='Restored Item'").fetchone()
...
class TestImportProducts:
    ...
    def test_import_creates_categories(self, app_ctx, translations_dir):
        ...
        row = get_db().execute("SELECT name FROM categories WHERE name='NewCat'").fetchone()
        assert row is not None
...
class TestRestoreScoreWeights:
    def test_restore_weights(self, app_ctx, db):
        ...
        cur = db.cursor()
        _restore_score_weights(cur, [
            {"field": "kcal", "enabled": 1, "weight": 75, "direction": "lower",
             "formula": "minmax", "formula_min": 0, "formula_max": 0},
        ])
        db.commit()
        row = db.execute("SELECT weight FROM score_weights WHERE field='kcal'").fetchone()
...
class TestRestoreCategories:
    def test_restore_categories(self, app_ctx, db, translations_dir):
        ...
        cur = db.cursor()
        pending = _restore_categories(cur, [
            {"name": "Fruits", "emoji": "🍎"},
        ])
        db.commit()
