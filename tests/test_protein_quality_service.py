...
class TestUpdateEntry:
    def test_update_pdcaas(self, app_ctx, translations_dir):
        ...
        row = get_db().execute("SELECT id FROM protein_quality WHERE name='whey'").fetchone()
        update_entry(row["id"], {"pdcaas": 0.95})
        updated = get_db().execute("SELECT pdcaas FROM protein_quality WHERE id=?", (row["id"],)).fetchone()
...
class TestDeleteEntry:
    def test_delete_existing(self, app_ctx, translations_dir):
        ...
        result = add_entry({
            "name": "to_delete",
            "keywords": ["delete"],
            "pdcaas": 0.1, "diaas": 0.1,
        })
        delete_entry(result["id"])
        from db import get_db
        row = get_db().execute("SELECT id FROM protein_quality WHERE id=?", (result["id"],)).fetchone()
        assert row is None
