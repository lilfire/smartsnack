...
def _build_prompt(text: str, target_lang: str) -> str:
    ...
    return (
        ...
        "Rules:\n"
        f"- Your entire output must be in {lang_name} only. "
        "There must be no words, characters, or scripts from any other "
        "language anywhere in your output — this includes European languages "
        "(Spanish, Italian, French, English, German, etc.) AND non-Latin "
        "scripts such as Chinese (漢字), Japanese (仮名), Korean (한글), "
        "Arabic, Cyrillic, and similar. "
        "Example of WRONG output: `palm油` (Chinese character in a word). "
        f"Example of CORRECT output: `palmolje` (fully in {lang_name}).\n"
        "- Translate ALL text, including text inside parentheses. "
        "Parenthetical content is part of the ingredient description — "
        "it must be translated, not preserved in the source language.\n"
        "- Do not add explanations, comments, or formatting.\n"
        "- Preserve the original structure: commas, semicolons, parentheses, "
        "and percentage values must stay in their original positions — but "
        "the WORDS inside them must be translated.\n"
        "- If the entire text is already correctly and completely in "
        f"{lang_name} (every single word), return it as-is. "
        "If any word is in a different language, translate the whole text.\n"
        "- Do not add or remove any ingredients.\n"
        "- Self-check: after writing your response, scan it for any word or "
        "character that is not exclusively in the "
        f"{lang_name} script — including embedded non-Latin characters "
        "(Chinese, Japanese, Korean, Arabic, Cyrillic, etc.). "
        "If found, translate the whole ingredient term before returning.\n\n"
        f"Ingredients:\n{text}"
    )
