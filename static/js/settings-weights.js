...
// categoryScopeData and getActiveScope are shared with settings-weights-render.js.
let _activeScope = '';
export const getActiveScope = () => _activeScope;
export const categoryScopeData = [];
let _allCategories = [];
...
// ── Renderer lives in settings-weights-render.js ──────
// Re-exported so existing importers (app.js, tests) keep working.
export { renderWeightItems, toggleWeightConfig } from './settings-weights-render.js';
import { renderWeightItems } from './settings-weights-render.js';
...
let _pendingWeightSave = false;
export async function saveWeights() {
  if (_weightSaving) { _pendingWeightSave = true; return; }
  _weightSaving = true;
  try {
    ...
    _weightSaving = false;
    if (_pendingWeightSave) { _pendingWeightSave = false; saveWeights(); }
  }
}
