...
// H8 regression (LSO-1684): early returns in saveProduct (merge-modal cancel,
// duplicate-check network error) used to skip the button-reset finally block,
// leaving the Save button permanently disabled on "Saving...".
describe('saveProduct re-enables the save button (H8)', () => {
  let saveBtn;
  ...
  beforeEach(() => {
    ...
    saveBtn.setAttribute('data-action', 'save-product');
    saveBtn.setAttribute('data-id', '1');
    saveBtn.textContent = 'btn_save';
    document.body.appendChild(saveBtn);
  });

  it('disables the button and shows saving label while the save runs', async () => {
    let resolveDup;
    api.mockImplementationOnce(() => new Promise((res) => { resolveDup = res; }));
    api.mockResolvedValueOnce({});
    const promise = saveProduct(1);
    expect(saveBtn.disabled).toBe(true);
    expect(saveBtn.textContent).toBe('toast_saving');
    resolveDup({ duplicate: null });
    await promise;
  });
  ...
  it('re-enables the button after user cancels the duplicate merge modal', async () => {
    ...
    expect(saveBtn.textContent).toBe('btn_save');
  });

  it('re-enables the button after a duplicate-check network error', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
  ...
  it('re-enables the button after a successful save', async () => {
    api.mockResolvedValueOnce({ duplicate: null }).mockResolvedValueOnce({});
    await saveProduct(1);
    expect(saveBtn.disabled).toBe(false);
    expect(saveBtn.textContent).toBe('btn_save');
  });

  it('re-enables the button after a PUT failure', async () => {
    vi.spyOn(console, 'error').mockImplementation(() => {});
    api.mockResolvedValueOnce({ duplicate: null }).mockRejectedValueOnce(new Error('save fail'));
    await saveProduct(1);
    expect(saveBtn.disabled).toBe(false);
    expect(saveBtn.textContent).toBe('btn_save');
    console.error.mockRestore();
